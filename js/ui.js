...
//  EMPOWER_LEVEL/EMPOWER_BONUS_SP/EMPOWER_COST live in shared/definitions.js
//  (read by the server's own empower handler, server/handlers/skills.js, from
//  the same module) so the requirement and the cost shown here can never
//  drift from what actually gets charged and granted. Nothing is reset by an
//  empowerment — it buys EMPOWER_BONUS_SP skill points and nothing else.
// ─────────────────────────────────────────────────────────
...
// (empowerment cost, VIP tiers, GRAM packs, season packs). It was copy-pasted as a
...
function updateEmpowerUI() {
  ...
  el.innerHTML = `
    ...
    <div style="padding:16px 12px 20px">
      ...
      <div class="upg-reset-hint">${tVars('empowerCountFmt', { n: player.empowers || 0 })}</div>
    </div>`;
}
...
function drawJoystick() {
  ...
  ctx.save();
  ctx.globalAlpha = held ? 0.95 : 0.66;
...
const _GRAM_SHOP_PKGS_UI = [
  ...
  // Усиление tab — pure material packs, one of each thing an empowerment bills
  // for (EMPOWER_COST, shared/definitions.js), norm_stone included. Mirrors
  // _GRAM_SHOP_PKGS (server/shop.js), which is what actually grants them; the
...
// that only grant items; the actual empowerment is still done from the
