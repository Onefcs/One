...
const _GAME_ONLY_BTNS = ['chat-btn', 'vip-btn', 'market-btn', 'gram-shop-btn', 'rating-btn', 'events-btn', 'special-btn'];
...
// ─────────────────────────────────────────────────────────
//  SPECIAL PACKS PANEL (Специальные) — one-time 12h GRAM sale, reached
//  from the Events button stack. The server owns the sale window
//  (SPECIAL_SALE_END_AT, server/index.js) and hands it over once as
//  gameStart's specialSale.endAt (see _applyGameStart in js/network.js);
//  everything here just counts it down locally and hides the entry point
//  once it's passed, the same pattern as the death battle countdown.
// ─────────────────────────────────────────────────────────
const _SPECIAL_PACKS_UI = [
  { id:'sp20',  gram:20,  get label() { return t('specialPkgLabel_sp20'); },  color:'#8fc95c',
    nexum:1000, blessStones:5,  normStones:10, potions:20, armor:'Uncommon', weapon:'Uncommon', enhance:3,
    skillBooks:{ each:2 }, gold:30000,  cloak:true },
  { id:'sp50',  gram:50,  get label() { return t('specialPkgLabel_sp50'); },  color:'#e5a546',
    nexum:2000, blessStones:10, normStones:10, potions:40, armor:'Uncommon', weapon:'Uncommon', enhance:7,
    skillBooks:{ each:3 }, gold:50000,  bonusSP:2, artifact:true },
  { id:'sp100', gram:100, get label() { return t('specialPkgLabel_sp100'); }, color:'#eb4e61',
    nexum:4000, blessStones:15, normStones:15, potions:70, armor:'Rare',     weapon:'Rare',     enhance:3,
    skillBooks:{ each:5 }, gold:300000, bonusSP:5, cloak:true, artifact:true },
];

function _specialSaleActive() {
  return !!window._specialSaleEndAt && Date.now() < window._specialSaleEndAt;
}

function showSpecialBtn() {
  const btn = document.getElementById('special-btn');
  if (!btn) return;
  if (!_specialSaleActive()) { btn.dataset.shown = ''; btn.style.display = 'none'; return; }
  btn.dataset.shown = '1';
  btn.style.display = (activeTab === 0) ? 'flex' : 'none';
  _positionSpecialBtn();
}
...
function _positionSpecialBtn() {
  ...
  const btn = document.getElementById('special-btn');
  if (!btn || !eventsBtn) return;
  const eTop = parseFloat(eventsBtn.style.top) || 0;
  btn.style.top       = (eTop + 28 + 4) + 'px';
  btn.style.left      = eventsBtn.style.left;
  btn.style.width     = eventsBtn.style.width;
  btn.style.right     = 'auto';
  btn.style.transform = 'none';
}

let _specialTimerTick = null;
function openSpecialPanel() {
  const panel = document.getElementById('special-panel');
  if (!panel) return;
  if (!_specialSaleActive()) { closeSpecialPanel(); return; }
  panel.style.display = 'flex';
  _renderSpecialPanel();
  clearInterval(_specialTimerTick);
  _specialTimerTick = setInterval(_tickSpecialTimer, 1000);
  _tickSpecialTimer();
}
...
function closeSpecialPanel() {
  ...
  if (panel) panel.style.display = 'none';
  clearInterval(_specialTimerTick);
  _specialTimerTick = null;
}

function _tickSpecialTimer() {
  const el = document.getElementById('special-panel-timer');
  if (!el) return;
  if (!_specialSaleActive()) {
    el.textContent = t('specialSaleExpiredLbl');
    clearInterval(_specialTimerTick);
    _specialTimerTick = null;
    showSpecialBtn();
    return;
  }
  const msLeft = window._specialSaleEndAt - Date.now();
  const s = Math.max(0, Math.floor(msLeft / 1000));
  const h = Math.floor(s / 3600), m = Math.floor((s % 3600) / 60), sec = s % 60;
  const hms = `${String(h).padStart(2, '0')}:${String(m).padStart(2, '0')}:${String(sec).padStart(2, '0')}`;
  el.textContent = tVars('specialSaleTimeLeftFmt', { time: hms });
}
...
function _renderSpecialPanel() {
  ...
  const bal = window._gramBalance || 0;
  el.innerHTML = `
    <div style="background:rgba(214,63,143,0.08);border:1px solid rgba(214,63,143,0.2);border-radius:10px;padding:10px 14px;margin-bottom:14px;font-size:13px;color:#ff8fc4;text-align:center">
      ${tVars('gramShopBalanceFmt', { bal: `<b>${bal.toFixed(7)}</b>` })}
    </div>
    ${_SPECIAL_PACKS_UI.map(pkg => _specialPackHtml(pkg, bal)).join('')}
  `;
}

// class-specific cloak/artifact icon paths — mirror shared/definitions.js's
// cloak_c_<class>/artifact_c_<class> img fields.
function _classCloakImg() {
  const cls = (player && player.type) || 'lev';
  return `/images/cloak/${cls}_c.png`;
}
function _classArtifactImg() {
  const cls = (player && player.type) || 'lev';
  return `/images/artifact/${cls}.png`;
}

function _specialPackHtml(pkg, bal) {
  const canAfford = bal >= pkg.gram;
  const kGold = pkg.gold >= 1000 ? (pkg.gold / 1000).toFixed(0) + 'k' : pkg.gold;

  // same ri() pattern as the Gram Shop / VIP cards
  function ri(img, label, cls) {
    return `<div class="vip-ri${cls ? ' vip-ri-' + cls : ''}"><img class="vip-ri-img" src="${img}"><span class="vip-ri-label">${label}</span></div>`;
  }
  ...
  // potions — all 6 kinds, same qty each
  rows += _POTION_NAMES.map(p => ri(`/images/potion/${p}.png`, `×${pkg.potions}`, '')).join('');

  // armor set, pre-enhanced
  if (pkg.armor) {
    const key = pkg.armor.toLowerCase();
    const icons = _ARMOR_ICONS[key] || [];
    const enhLbl = pkg.enhance ? `+${pkg.enhance}` : '';
  ...
  // weapon — class-specific, pre-enhanced
  if (pkg.weapon) {
    const key = pkg.weapon.toLowerCase();
    const pfx = wepPfxMap[key] || 'c';
    rows += ri(`/images/wep/${pfx}${wepSfx}.png`, pkg.enhance ? `+${pkg.enhance}` : '', key);
  ...
  if (pkg.blessStones) rows += ri('/images/bless.png', `×${pkg.blessStones}`, 'rare');
  if (pkg.normStones)  rows += ri('/images/norm.png',  `×${pkg.normStones}`, '');
  ...
  // skill books — for the buyer's own class
  if (pkg.skillBooks) {
    const bookUri = `data:image/svg+xml,<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 24 24' fill='none' stroke='%23e3941d' stroke-width='2' stroke-linecap='round' stroke-linejoin='round'><path d='M4 4.5A2.5 2.5 0 0 1 6.5 2H20v17H6.5A2.5 2.5 0 0 0 4 21.5'/><path d='M4 4.5v17'/><line x1='9' y1='7' x2='16' y2='7'/><line x1='9' y1='11' x2='16' y2='11'/></svg>`;
    rows += ri(bookUri, _skillBooksLabel(pkg.skillBooks), 'epic');
  }
  ...
  // class cloak / artifact
  if (pkg.cloak)    rows += ri(_classCloakImg(),    t('classCloakShortLbl'), '');
  if (pkg.artifact) rows += ri(_classArtifactImg(), t('classArtifactShortLbl'), '');

  return `<div class="special-pack-card" style="border-color:${pkg.color}44">
    <div class="special-pack-card-head">
      <div>
        <div class="special-pack-title" style="color:${pkg.color}">${pkg.label}</div>
        <div class="special-pack-price">${pkg.gram} GRAM</div>
      </div>
      <button class="special-pack-buy-btn${canAfford ? '' : ' disabled'}"
        style="border-color:${pkg.color};color:${canAfford ? pkg.color : '#645f57'}"
        onclick="${canAfford ? `openSpecialPackConfirm('${pkg.id}')` : ''}">
        ${canAfford ? t('affordableBuyBtn') : t('notEnoughBtn')}
      </button>
    </div>
    <div class="vip-items-row">${rows}</div>
...
function openSpecialPackConfirm(pkgId) {
  const pkg = _SPECIAL_PACKS_UI.find(p => p.id === pkgId);
  if (!pkg) return;
  const bal = window._gramBalance || 0;
  if (bal < pkg.gram) return;
  const existing = document.getElementById('special-confirm-ov');
  if (existing) existing.remove();
  const kGold = pkg.gold >= 1000 ? (pkg.gold / 1000).toFixed(0) + 'k' : pkg.gold;
  const enhSuffix  = pkg.enhance ? ` +${pkg.enhance}` : '';
  const armorLine  = pkg.armor  ? `<div style="color:#c5bfb7">${tVars('fullArmorSetFmt', { rarity: pkg.armor })}${enhSuffix}</div>` : '';
  const weaponLine = pkg.weapon ? `<div style="color:#c5bfb7">${tVars('classWeaponFmt', { rarity: pkg.weapon })}${enhSuffix}</div>` : '';
  const stoneLine  = (pkg.blessStones || pkg.normStones)
    ? `<div style="color:#c5bfb7">• ${pkg.blessStones || 0}× ${t('safeStonesShortLbl')}, ${pkg.normStones || 0}× ${t('normalStonesShortLbl')}</div>` : '';
  const spLine     = pkg.bonusSP ? `<div style="color:#c5bfb7">${tVars('bonusSkillPointsFmt', { n: pkg.bonusSP })}</div>` : '';
  const bookLine   = pkg.skillBooks ? `<div style="color:#c5bfb7">• ${_skillBooksLabel(pkg.skillBooks)} ${t('classBooksSuffix')}</div>` : '';
  const cloakLine  = pkg.cloak    ? `<div style="color:#c5bfb7">• ${t('classCloakShortLbl')}</div>` : '';
  const artLine    = pkg.artifact ? `<div style="color:#c5bfb7">• ${t('classArtifactShortLbl')}</div>` : '';
  const nexumLine  = pkg.nexum ? `<div style="color:#6fc7ff">• +${pkg.nexum} Liberty</div>` : '';
  const ov = document.createElement('div');
  ov.className = 'market-modal-overlay';
  ov.id = 'special-confirm-ov';
  ov.onclick = () => ov.remove();
  ov.innerHTML = `
    <div class="market-modal-sheet" onclick="event.stopPropagation()">
      <div style="display:flex;align-items:center;margin-bottom:14px">
        <div style="font-size:16px;font-weight:800;color:${pkg.color}">${pkg.label} — ${pkg.gram} GRAM</div>
        <button onclick="document.getElementById('special-confirm-ov').remove()" style="margin-left:auto;width:28px;height:28px;border:none;border-radius:50%;background:rgba(209,204,197,.08);color:#968a7a;cursor:pointer">✕</button>
      </div>
      <div style="background:rgba(209,204,197,.04);border-radius:10px;padding:12px 14px;margin-bottom:14px;font-size:13px;line-height:1.8">
        <div style="color:#c5bfb7">${tVars('goldAmountFmt', { n: kGold })}</div>
        <div style="color:#c5bfb7">${tVars('eachPotionFmt', { n: pkg.potions })}</div>
        ${armorLine}${weaponLine}${stoneLine}${spLine}${bookLine}${cloakLine}${artLine}${nexumLine}
      </div>
      <div style="display:flex;justify-content:space-between;font-size:13px;margin-bottom:16px">
        <span style="color:#b2a288">${t('costLbl')}</span>
        <span style="font-weight:700;color:${pkg.color}">${pkg.gram} GRAM</span>
      ...
      <button class="gram-btn gram-btn-green" style="width:100%;padding:13px"
        onclick="_confirmSpecialPackBuy('${pkgId}')">${tVars('buyForFmt', { price: pkg.gram })}</button>
    </div>`;
  document.body.appendChild(ov);
}

function _confirmSpecialPackBuy(pkgId) {
  const ov = document.getElementById('special-confirm-ov');
  if (ov) ov.remove();
  if (typeof netSpecialPackBuy === 'function') netSpecialPackBuy(pkgId);
}
...
function onSpecialPackError(msg) {
  _marketToast(msg || t('purchaseErrorLbl'), 'err');
}
