...
// removes it, grants the flat EMPOWER_BONUS_SP and answers with 'empowerDone'
// (see the handler below). Nothing about the character is reset.
function netEmpower() {
...
function _initPetCraftHandlers(s) {
  ...
  s.on('upgradesReset', ({ pointsReturned, keptSP, newNexumBalance }) => {
    ...
    player.upgrades = {};
    // Emptying the map ends any commitment keptSP was carrying, so the points
  ...
  // — the server's own _commitServerItems call inside the empower handler
  // already pushed the item-cost removal; this only carries the reward.
  //
  // Nothing else about the character moves: an empowerment leaves the level,
  // the XP and the upgrades exactly where they were, so bonusSP and the
  // counter are the whole payload.
  s.on('empowerDone', ({ bonusSP, empowers } = {}) => {
    if (!player) return;
    if (Number.isFinite(bonusSP)) player.bonusSP = bonusSP;
    if (Number.isFinite(empowers)) player.empowers = empowers;
    if (typeof netSaveProgress === 'function') netSaveProgress();
