...
// The counter itself. Перерождение stored its count in `rebirths`; the price
// ladder reads `empowers`, and the two are the same tally — a player who
// rebirthed four times is four empowerments in, so their next one is the 5th
// and costs double. Carried over rather than restarted, which is also what
// keeps "Усилений пройдено" from dropping to zero for them.
//
// Returns null when there is nothing to do. Idempotent by construction: a
// record that carries an `empowers` field at all — including empowers:0 — is
// never touched again.
function migrateEmpowers(st) {
  if (!st || typeof st !== 'object') return null;
  if (st.empowers != null) return null;
  return { empowers: Math.max(0, Math.floor(Number(st.rebirths)) || 0) };
}
