...
class Room {
  ...
  get dungeonData() {
    ...
    // ascent only ever carries `bounds` here (see generateAscent's own
    // comment) — `lanes` is per-player instance state Room.js reads
    // directly off this._dungeon.ascent, never meant for the wire.
    return { gridPacked: this._gridPacked, rooms: d.rooms, spawn: d.spawn, w: d.w, h: d.h, safeZone: d.safeZone, armEntries: d.armEntries, farmZoneEntry: d.farmZoneEntry, returnPad: d.returnPad, corridorGates: d.corridorGates, race10: d.race10, guildWar: d.guildWar, farmZone: d.farmZone, ascent: d.ascent ? { bounds: d.ascent.bounds } : undefined };
  }
