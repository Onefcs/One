'use strict';
// Страх (Fear) — a private, on-demand wave-survival instance.
//
// Second of the timed/instanced event machines to leave server/index.js, on
// the pattern server/events/guildwar.js set. Four names come in from the
// caller; everything else it needs, it requires for itself:
//
//   io               fan-out to a hall or a socket
...
//
// The export surface is wide — sixteen symbols, every one of which
// server/index.js genuinely uses — because the socket handlers that drive a run
// still live there. That is the coupling as it actually is; naming it here is
// the point, since a boundary you can read is what the handlers can later be
// moved across.
const Room = require('../game/Room');
const { FLOOR_IDS } = require('../game/floors');
const { FEAR_MAX_WAVE } = require('../../shared/definitions');

// See createGuildWar for why this is checked rather than assumed: a name missed
// in the wiring is otherwise undefined at construction and only throws when
// that path first runs.
const REQUIRED_DEPS = ['io', 'safeTimeout', '_returnToHub', '_socketTid'];

module.exports = function createFear(deps) {
  const missing = REQUIRED_DEPS.filter(k => !deps || deps[k] == null);
  if (missing.length) throw new Error(`createFear: missing deps: ${missing.join(', ')}`);
  const { io, safeTimeout, _returnToHub, _socketTid } = deps;
  ...
  return {
    // FEAR_RECONNECT_GRACE_MS stays private: only this file reads it, and
    // Room.js keeps its own copy in sync by comment rather than by import.
