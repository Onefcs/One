'use strict';
// Кровавая Башня — the scheduled corridor race: a registration window, one
// private corridor per entrant, a shared boss at the top, and the payout.
//
// Fifth machine out of server/index.js, on the pattern server/events/
// guildwar.js set. Twelve names come in, more than any of the others so far,
// and the reason is worth naming: the daily-attempt ledger
// (_lockRace10Daily/_race10AttemptsLeft) and the PvP history writer live
// against the database and the session, not against this event, so they are
// borrowed rather than owned.
//
//   io                    fan-out to the tower, the entrants or one socket
...
//   _lockRace10Daily      spend one of today's attempts
//   _race10AttemptsLeft   how many are left today
//
// `_race10` goes back to the caller as the same state object: server/index.js
// reads it from thirty-six places and Арена 3х3 reads it too, which is why the
// arena moves last.
...
module.exports = function createRace10(deps) {
  ...
  if (missing.length) throw new Error(`createRace10: missing deps: ${missing.join(', ')}`);
  const {
    io, safeTimeout, getRoom, notifyEventSoon, notifyEventStarted,
    logPlayer, _returnToHub, _findPlayerAnyFloor, _socketTid,
    _recordPvpHistory, _lockRace10Daily, _race10AttemptsLeft,
  ...
  return {
    // RACE10_REG_MS/RACE10_MAX_MS stay private: server/index.js names them in
    // comments only, so exporting them would invent a caller.
