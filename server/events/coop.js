'use strict';
// Сотрудничество (Coop) — a private two-player instance and the pre-run lobby
// that forms the pair.
//
// Third machine out of server/index.js, on the pattern server/events/
// guildwar.js set. It is the least entangled of them: two names come in.
//
//   io              fan-out to a run, a lobby or a socket
//   _returnToHub    put a socket back in the hub
//
// _coopRooms and _trackCoopRoom stay private — nothing outside this file ever
// touched them. Everything else is exported because the socket handlers that
// drive a run and a lobby still live in server/index.js.
...
module.exports = function createCoop(deps) {
  ...
  if (missing.length) throw new Error(`createCoop: missing deps: ${missing.join(', ')}`);
