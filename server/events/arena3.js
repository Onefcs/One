'use strict';
// Арена 3х3 — queue-driven team PvP: six players sign up during a window, get
// split at random into two teams of three, and fight until one side is out.
//
// Last of the six event machines to leave server/index.js, and the one whose
// measurement was worth the wait. Asked what the region it lived in needed,
// eslint's no-undef returned thirty-one names, among them _db, _dbBroadcast,
// _dbEliminate, _dbFrozen, _race10, _race10Broadcast, _race10Eliminate,
// _race10Frozen, _coopEliminate, _coopEjectOnDisconnect, _fearEliminate and
// _fearHoldOnDisconnect — which read as the arena reaching into every other
// event in the game.
//
// It was not. Parked in the middle of the arena's own code were six helpers
// that coordinate ACROSS the machines and belong to none of them:
// _socketTid, _reclaimQueues, _rekeyQueue, _teleportCastFrozen, _pvpFrozen
// and _pvpEliminate. They stay in server/index.js, which is where a
// coordinator belongs. With them left behind the arena needs nineteen names,
// none of them another machine's internals — the difference between the two
// numbers is the whole reason this file was cut last rather than first.
//
// Thirteen of those nineteen it requires for itself. What comes in:
//
//   io                    fan-out to the arena, the queue or one socket
...
//   logPlayer             the per-account event log
...
//   _lockArena3Daily      spend one of today's attempts
//   _arena3AttemptsLeft   how many are left today
//   DAILY_DUNGEON_ATTEMPTS  the per-day cap the two share, for the UI
...
module.exports = function createArena3(deps) {
  ...
  if (missing.length) throw new Error(`createArena3: missing deps: ${missing.join(', ')}`);
  const {
    io, safeTimeout, getRoom, notifyEventSoon, notifyEventStarted,
    logPlayer, _returnToHub, _findPlayerAnyFloor, _socketTid,
    _recordPvpHistory, _lockArena3Daily, _arena3AttemptsLeft,
