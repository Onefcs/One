'use strict';
// Война гильдий — the clan tower's window, its capture, and the income it pays
// out while a clan holds it.
//
// First of the timed-event state machines to move out of server/index.js. Each
// of them is already a closed loop around one `const` state object plus the
// timers that drive it, so the only thing the monolith was really providing was
// ambient access to six names. They are parameters now, listed in one place:
//
//   io                     fan-out to a floor or a socket
//   safeTimeout            a timer that cannot take the process down
//   notifyEventSoon        the 30-minute warning over the bot
//   notifyEventStarted     the "it's open" announcement
//   _socketForTelegramId   live socket for an account, if it has one
//   playerFloorMap         which floor a socket is standing on
//
// That list is the point of the split, not the line count: SCALING.md's plan
// for a second process turns exactly these into things a worker must be handed
// rather than things it happens to have. The state object is still handed back
// to the caller as-is — server/index.js pokes `_gw` directly from fifteen
// places, and rewriting those is a separate change from moving the machine.
const ClanModel = require('../models/Clan');
const GuildWarStateModel = require('../models/GuildWarState');
const { FLOOR_IDS } = require('../game/floors');
const {
  EVENT_NOTIFY_BEFORE_MS, nextEventStartAt, UNIQUE_SHARDS,
  GUILD_WAR_DAYS_MSK, GUILD_WAR_HOURS_MSK, GUILD_WAR_WINDOW_MS,
  GUILD_WAR_TOWER_HP, GUILD_WAR_SHARD_MIN, GUILD_WAR_SHARD_MAX,
  GUILD_WAR_INCOME_INTERVAL_MS,
} = require('../../shared/definitions');

// Every name the machine below needs from its caller. Checked rather than
// assumed: a missing dependency is otherwise `undefined` at construction and
// only throws whenever that path first runs — which for a weekly event can be
// days after the deploy that broke it, in front of players. Failing here means
...
module.exports = function createGuildWar(deps) {
  ...
  if (missing.length) throw new Error(`createGuildWar: missing deps: ${missing.join(', ')}`);
  const {
    io, safeTimeout, notifyEventSoon, notifyEventStarted,
    _socketForTelegramId, playerFloorMap,
  } = deps;
