...
const _GRAM_SHOP_PKGS = [
  ...
  // Усиление tab — pure material packs, one of each thing an empowerment
  // bills for (EMPOWER_COST, shared/definitions.js), norm_stone included.
  // These only GRANT the listed items (via the same pkg.boxes/pkg.stones
  // handling every other package already uses below — `stones` isn't
  // stone-specific, it resolves any CRAFT_MATS id, which is how rece/recl
  // land here too); buying one never performs an empowerment by itself, that's
  // still the separate 'empower' event above, spending materials out of the
  // inventory these packs fill.
  //
