'use strict';
// What the GRAM shop and the VIP tiers hand out, and how much inventory room
// that needs.
//
// Tables and pure functions over them — no session, no socket, no database, and
// nothing here grants anything by itself. The handlers in server/index.js
// (gramShopBuy, claimVipRewards) read these to decide what to give; keeping the
// tables and the room arithmetic in one file is what stops the two drifting
// apart, which is the failure this data has actually had: a package that grants
// more than the space check reserved for it destroys the overflow silently.
//
// The weapon/armor entries are a server-side subset of js/definitions.js —
// deliberately duplicated rather than imported, since the client's copy is
// whatever the player's tab last loaded.
...
module.exports = {
  // _VIP_WEAPONS stays private: only _vipLevelItems below reads it.
  _VIP_BP, _STONE_DEFS,
  _GRAM_SHOP_PKGS, _SEASON_SHOP_PKGS, _SPECIAL_SHOP_PKGS,
  _SHOP_CLASS_WEAPONS, _SHOP_ARMOR_SETS,
  _shopNewSlots, _vipLevelItems, _vipGoldReward,
};
