'use strict';
// Kill-loot rolls: what a monster drops, and into whose inventory.
//
// Split out of server/index.js because these are the shape that file has the
// least of — pure functions over the shared catalog and a caller-supplied
// inventory, with no session, no socket and no database. The only impurity is
// Math.random(), which is the point of them.
//
// They mutate `inv` in place via _invAdd and return the granted list; the
// caller (the 'attack'/'skillAttack' handlers in server/index.js) decides WHO
// a roll runs for — loot-winner arbitration among a party — and reports the
// result back to that client.
//
// Server-side rolling is the whole reason they exist: all three used to be
// client-rolled and reached the server only inside the next saveProgress blob,
// which _canonSavedItem trusts for any valid id+enhance+qty. That was the
// single biggest "items appearing out of nowhere" vector in the game, since it
// fires on every kill.
const {
  ITEM_DEF, CRAFT_MATS, BOX_DEF, ENEMY_DEF, UNIQUE_SHARDS,
  UNIQUE_SHARD_MIN_LEVEL, UNIQUE_SHARD_CHANCE, UNIQUE_SHARD_MAX_QTY,
  FARM_SHARD_CHANCE, FARM_ADV_SKILL_BOOK_CHANCE,
  FARM_NORM_STONE_CHANCE, FARM_BLESS_STONE_CHANCE,
  FARM_SPECIES_BOOKS, FARM_SPECIES_SHARDS,
  ...
  FARM2_ADV_SKILL_BOOK_CHANCE, FARM2_UNIQUE_WEAPON_CHANCE,
  BOSS_ITEM_DROP_MULT, itemDropChanceAtLevel, itemRarityForLevel,
  dropLevelGapDivisor, roomDropMult, roomKeyChance, roomEnchantStoneChance,
...
// Called from the 'attack'/'skillAttack' handlers in server/index.js; see this
// file's header for who decides which player a roll runs for.
