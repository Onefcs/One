'use strict';
// craft: the safeOn handlers moved out of server/index.js verbatim, with
...
module.exports = function registerCraft(s, safeOn, deps) {
  const {
  ...
  const {
    ...
    safeOn('craftStone', ({ matId } = {}) => {
      if (!s.authed) return;
      logPlayer(s.authed.telegramId, s.authed.username, 'stone_craft_removed', { matId });
    ...
    safeOn('craftGear', async ({ itemId } = {}) => {
      ...
      try {
        ...
        if (rec.nexumCost && activeSessions.get(s.authed.telegramId) !== socket.id) {
          ...
          _target.emit('gearCrafted', {
            ...
            newNexumBalance: s.nexumBalance, delivered: _res ? _res.delivered : false,
          });
          return;
        }
        ...
        socket.emit('gearCrafted', { itemId, success, resultEnhance: success ? resultEnhance : 0, newNexumBalance: s.nexumBalance });
      } catch (err) {
        console.error('craftGear:', err);
        logPlayerErr(s.authed.telegramId, s.authed.username, 'gear_craft', err, { itemId });
    ...
    // the in-memory s.lastStats.inventory, not a server-tracked balance), so
    ...
    safeOn('enhanceItem', ({ id, enhance, stoneType, slot } = {}) => {
      ...
      const _enhNotFound = where => {
        logPlayer(s.authed.telegramId, s.authed.username, 'enhance_not_found', {
          id, wantEnhance: curEnh, where, rev: s.invRev, invLen: inv.length,
      ...
      const eq = s.lastStats.equipment || {};
      ...
      if (slot && target.slot !== slot) {
        logPlayer(s.authed.telegramId, s.authed.username, 'enhance_slot_mismatch',
      ...
      // moved between the inventory and an equip slot inside s.lastStats with no
      ...
      if (success) {
        ...
        if (targetSlot) s.lastStats.equipment[targetSlot] = null;
      ...
      _commitServerItems(inv, s.lastStats.equipment || {}, 'enhance',
    ...
    safeOn('openLootBox', ({ id } = {}) => {
      ...
      const charClass = s.lastStats.type || 'lev';
    ...
    safeOn('craftPet', async ({ rarity } = {}) => {
      if (!s.authed) return;
      // s.itemOpBusy, like every other handler that holds an inventory across an
      // await: this one was the only craft without it, so a saveProgress landing
      // between the balance spend and the commit below could replace s.lastStats
      ...
      try {
        ...
        if (s.lastStats.inventory.length >= SERVER_INV_MAX) {
        ...
        // closure's s.lastStats belongs to a session nobody's client can see.
        ...
        if (activeSessions.get(s.authed.telegramId) !== socket.id) {
          ...
          _target.emit('petCrafted', {
            pet: resultPet, newNexumBalance: s.nexumBalance, delivered: !!_res && !!resultPet,
          });
          return;
        }

        const _delivered = resultPet ? _invAdd(s.lastStats.inventory, resultPet) : false;
        _commitServerItems(s.lastStats.inventory, null, 'pet_craft',
          { rarity, cost: rec.nexumCost, got: resultPet ? resultPet.id : null }, { beforeLen: _beforeLen });

        socket.emit('petCrafted', {
          pet: resultPet, newNexumBalance: s.nexumBalance, delivered: _delivered,
        });
      } catch (err) {
        console.error('craftPet:', err);
        logPlayerErr(s.authed.telegramId, s.authed.username, 'pet_craft', err, { rarity });
      ...
      try {
        ...
        if (activeSessions.get(s.authed.telegramId) !== socket.id) {
          ...
          _target.emit('classGearCrafted', {
            item: resultItem, newNexumBalance: s.nexumBalance, delivered: _res ? _res.delivered : false,
          });
          return;
        }
        ...
        socket.emit('classGearCrafted', { item: resultItem, newNexumBalance: s.nexumBalance, delivered: _delivered });
      } catch (err) {
        console.error('craftClassGear:', err);
        logPlayerErr(s.authed.telegramId, s.authed.username, 'class_gear_craft', err, { slot, rarity });
