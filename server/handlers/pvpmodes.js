'use strict';
// pvpmodes: the safeOn handlers moved out of server/index.js verbatim, with
...
module.exports = function registerPvpmodes(s, safeOn, deps) {
  ...
  const {
    _atkAllowed, _doEnterLocation, _emitNearby, socket,
  } = s;
    ...
    function _isPvpImmune(attackerId, targetId) {
      ...
      const gwA = s.currentRoom?.players.get(attackerId);
      const gwT = s.currentRoom?.players.get(targetId);
      ...
      const aPlayer = s.currentRoom?.players.get(attackerId);
      const tPlayer = s.currentRoom?.players.get(targetId);
    ...
    safeOn('pvpAttack', ({ targetId } = {}) => {
      ...
      const result = s.currentRoom.pvpAttack(socket.id, targetId);
      if (!result) return;
      ...
      if (result.hp <= 0) { io.to(targetId).emit('playerHurt', { id: targetId, hp: 0 }); _pvpEliminate(targetId, socket.id, s.currentRoom); }
    });
    ...
    safeOn('pvpSkillAttack', ({ targetId, key } = {}) => {
      ...
      if (_pvpFrozen(socket.id) || _pvpFrozen(targetId)) return;
      if (_isPvpImmune(socket.id, targetId)) return;
      const result = s.currentRoom.pvpSkillAttack(socket.id, targetId, key);
      if (!result) return;
      io.to(targetId).emit('pvpDamage', { dmg: result.dmg, hp: result.hp });
      socket.emit('pvpHit', { x: result.x, y: result.y, dmg: result.dmg, isCrit: result.isCrit, targetId });
      if (result.hp <= 0) { io.to(targetId).emit('playerHurt', { id: targetId, hp: 0 }); _pvpEliminate(targetId, socket.id, s.currentRoom); }
    ...
    safeOn('pvpSkillCC', ({ targetId, type, duration } = {}) => {
      ...
      if (_pvpFrozen(socket.id) || _pvpFrozen(targetId)) return;
      if (_isPvpImmune(socket.id, targetId)) return;
      const attacker = s.currentRoom.players.get(socket.id);
      if (!attacker || !attacker.pvpMode) return;
      if (attacker.hp <= 0) return;
      if (s.currentRoom.isPlayerInSafeZone(socket.id)) return;
      const target = s.currentRoom.players.get(targetId);
      if (!target || target.hp <= 0) return;
      if (s.currentRoom.isPlayerInSafeZone(targetId)) return;
    ...
    safeOn('deathBattleRegister', () => {
      ...
      const cp = s.currentRoom?.players.get(socket.id);
      ...
      _db.reg.set(socket.id, { name: s.authed.username, tid: s.authed.telegramId });
    ...
    safeOn('arena3Register', async () => {
      ...
      const lvl = (s.lastStats && s.lastStats.lvl) || 1;
      ...
      _a3.queue.set(socket.id, { name: s.authed.username, lvl, tid: s.authed.telegramId });
    ...
    safeOn('race10Register', async () => {
      ...
      _race10.queue.set(socket.id, { name: s.authed.username, lvl, tid: s.authed.telegramId });
    ...
    safeOn('fearEnter', async () => {
      ...
      const spot = s.currentRoom.fearDeploy(socket.id);
      if (!spot) {
    ...
    safeOn('setPvpMode', ({ pvpMode } = {}) => {
      if (s.currentRoom) s.currentRoom.setPlayerPvpMode(socket.id, pvpMode);
    });
};
