'use strict';
// coopfarm2: the safeOn handlers moved out of server/index.js verbatim, with
...
module.exports = function registerCoopfarm2(s, safeOn, deps) {
  ...
  const {
    ...
    safeOn('coopGroupCreate', async () => {
      ...
      _coopGroups.set(socket.id, { leaderName: s.authed.username, memberId: null, memberName: null });
    ...
    safeOn('coopGroupJoin', ({ leaderId } = {}) => {
      ...
      g.memberName = s.authed.username;
    ...
    safeOn('farm2GroupCreate', async () => {
      ...
      _farm2Groups.set(socket.id, { leaderName: s.authed.username, members: new Map() });
    ...
    safeOn('farm2GroupJoin', async ({ leaderId } = {}) => {
      ...
      g.members.set(socket.id, s.authed.username);
    ...
    safeOn('farm2GroupStart', async () => {
      ...
      const allNames = [s.authed.username, ...memberIds.map(id => g.members.get(id))];
